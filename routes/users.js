...
router.get("/donations", function (req, res, next) {
  // /api/donations comes from app.js that connects through express my routes to the front end
...
router.post("/", function (req, res, next) {
  ...
  if (!item || !itemPrice || !itemUrl || !itemDescription || !itemImg) {
    return res
      .status(400)
      .json({ message: "Item and Item price are required" });
  }

  db(
    `INSERT INTO donations (item, itemPrice, itemUrl,itemDescription, itemImg) VALUES ('${item}', ${itemPrice})`
  )
    .then(() => {
